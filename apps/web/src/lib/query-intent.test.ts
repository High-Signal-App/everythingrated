...
test('query-intent', () => {
  ...
  assert.ok(
    chat.tags.some((t) => t.key === 'realtime'),
    'detects realtime constraint'
  );
  // "budget" → free-tier tag
  assert.ok(
    chat.tags.some((t) => t.key === 'pricing' && t.value === 'free-tier'),
    'detects budget'
  );
  ...
  assert.ok(
    ts.tags.some((t) => t.key === 'language' && t.value === 'typescript'),
    'detects typescript'
  );
