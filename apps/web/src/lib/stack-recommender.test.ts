...
test('stack-recommender', () => {
    ...
    directories: [
      {
        ...
        name: 'Databases',
        items: [
          mkItem('d1', 'databases', 'Databases', { speed: 5, cost: 5, dx: 3 }, [
            { key: 'runs-on', value: 'cloudflare' },
          ]),
  ...
  assert.equal(
    recommendStack(catalogue, dxIntent).picks[0].top.item.slug,
    'bigsql',
    'dx priority picks bigsql'
  );

  // ── Constraint tag boost breaks ties / lifts a match ──────────────
  // Neutral weights but require cloudflare → d1 (tagged) wins via boost.
  const cfIntent: StackIntent = {
    directories: ['databases'],
    weights: {},
    tags: [{ key: 'runs-on', value: 'cloudflare' }],
  };
  ...
  assert.ok(
    Math.abs(d1.score - d1.base * (1 + TAG_BOOST)) < 1e-9,
    'tag boost applied multiplicatively'
  );

  // ── Cross-stack: one pick per requested directory, in order ───────
  const stackIntent: StackIntent = {
    directories: ['databases', 'auth-platforms'],
    weights: { dx: 3 },
    tags: [],
  };
  ...
  assert.deepEqual(
    stack.picks.map((p) => p.directorySlug),
    ['databases', 'auth-platforms'],
    'preserves order'
  );
  assert.equal(stack.picks[0].alternatives.length, 1, 'surfaces alternatives');

  // ── Unknown directory recorded, not crashed ───────────────────────
  const unknown = recommendStack(catalogue, {
    directories: ['nonexistent', 'databases'],
    weights: {},
    tags: [],
  });
