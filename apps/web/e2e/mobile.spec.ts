...
test.describe('mobile viewport — 390px', () => {
  ...
  test('landing shows hero, features, and a CTA', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /multi-axis ratings/i }).first()).toBeVisible();
    await expect(page.getByRole('heading', { name: /why multi-axis/i })).toBeVisible();
    await expect(page.getByRole('heading', { name: /how it works/i })).toBeVisible();
    await expect(page.getByRole('link', { name: /browse directories/i })).toBeVisible();
  });
