...
export const directorySubmissions = sqliteTable(
  ...
  (t) => ({
    statusCreated: index('directory_submissions_status_created_idx').on(t.status, t.createdAt),
  })
);
...
export const itemSubmissions = sqliteTable(
  ...
  (t) => ({
    dirSlugPending: uniqueIndex('item_submissions_dir_slug_active_idx').on(t.directoryId, t.slug), // partial unique in app layer: only when status in (pending, approved)
    statusCreated: index('item_submissions_status_created_idx').on(t.status, t.createdAt),
